...
func (me *CORESAN) Rewrite(txtCSNID string, txtFilename string, dblExpire float64, buffer []byte) (*jsons.JSONObject, error) {
	...
	dataNodeItem := me.MapDataNode[jsoInfo.GetString("txt_datanode")]
	me.MutexMapDataNode.RUnlock()

	jsoReq := drivers.Rewrite(dataNodeItem.JSOHost, txtCSNID, txtFilename, dblExpire, buffer)
